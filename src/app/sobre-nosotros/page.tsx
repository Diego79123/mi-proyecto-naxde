...
export default function SobreNosotrosPage() {
  ...
  const mockTeam = [
    ...
    },
    {
      ...
      bio: 'Artesanos de la tagua que fusionan la tradición colombiana con experiencias digitales inmersivas.'
    },
    {
      id: 'marcos-silva',
      name: 'Marcos Silva',
      ...
      <section className="py-24 px-6 md:px-12 lg:px-16">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            {[
              { title: "Propósito Superior", desc: "Impulsar el futuro de nuestra región", icon: Heart },
      ...
      <section className="py-24 md:py-40 bg-black overflow-hidden px-6 md:px-12 lg:px-16">
        <div className="max-w-screen-2xl mx-auto">
                      ...
                      {isActive && (
                        <div className="absolute inset-x-0 bottom-0 p-10 md:p-16 flex flex-col md:flex-row justify-between items-end gap-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
                          <div className="space-y-2">
                            <h4 className="text-3xl md:text-5xl font-headline font-black text-white leading-none uppercase tracking-tighter">{member.name}</h4>
                            <p className="text-primary font-bold text-xs md:text-sm uppercase tracking-[0.3em]">{member.role}</p>
                          </div>
          ...
          <div className="flex justify-center gap-12 mt-12 px-6">
            <button onClick={() => api?.scrollPrev()} className="group flex items-center gap-4 text-white/30 hover:text-white transition-all">
              <div className="w-1 h-1 rounded-full bg-current" />
              <span className="text-[10px] font-black uppercase tracking-[0.4em]">Anterior</span>
            </button>
            <div className="flex gap-2 items-center">
              {displayTeam.map((_, i) => (
                <div key={i} className={cn("h-1 transition-all duration-500 rounded-full", current === i ? "w-8 bg-primary" : "w-2 bg-white/10")} />
              ))}
            </div>
            <button onClick={() => api?.scrollNext()} className="group flex items-center gap-4 text-white/30 hover:text-white transition-all">
